...
export default class ElementCore implements FlexSubject {
  ...
  private isZContext(): boolean {
    return (
      this._forceZIndexContext || this._renderToTextureEnabled || this._zIndex !== 0 || this._isRoot || !this._parent
    );
  }
        ...
        if (recalc & 6) {
          ...
          if (!this._renderToTextureEnabled || !this._texturizer || !this._texturizer.renderOffscreen) {
            ...
            if (scissor && (scissor[2] <= 0 || scissor[3] <= 0)) {
              ...
              if (scissor[0] > ex || scissor[1] > ey || sx > scissor[0] + scissor[2] || sy > scissor[1] + scissor[3]) {
                this._outOfBounds = 1;
              }
  ...
  sortZIndexedChildren() {
    ...
    if (m) {
      ...
      if (!amount) {
        ...
        do {
          const v =
            a[i]._zIndex === b[j]._zIndex ? a[i]._updateTreeOrder - b[j]._updateTreeOrder : a[i]._zIndex - b[j]._zIndex;
