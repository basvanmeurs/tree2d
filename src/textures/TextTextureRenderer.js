...
export default class TextTextureRenderer {
    ...
    _draw() {
        ...
        if (this._settings.maxLines && lines.length > this._settings.maxLines) {
            ...
            if (this._settings.maxLinesSuffix) {
                ...
                const w = this._settings.maxLinesSuffix
                    ? this._context.measureText(this._settings.maxLinesSuffix).width
                    : 0;
                const al = this.wrapText(usedLines[usedLines.length - 1], wordWrapWidth - w);
        ...
        for (let i = 0; i < lines.length; i++) {
            const lineWidth = this._context.measureText(lines[i]).width;
        ...
        if (this._settings.highlight) {
            const color = this._settings.highlightColor || 0x00000000;

            const hlHeight = this._settings.highlightHeight * precision || fontSize * 1.5;
    ...
    wrapText(text, wordWrapWidth) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            for (let j = 0; j < words.length; j++) {
                ...
                const wordWidthWithSpace = wordWidth + this._context.measureText(" ").width;
